...
public final class BigDecimalMath
{
    ...
    private static MathContext context = new MathContext(50, RoundingMode.HALF_UP);

    public static MathContext getPrecision() {
        return context;
    }

    public static void setPrecision(MathContext context) {
        BigDecimalMath.context = Arguments.checkNull(context, "context");
    }

    public static void setPrecision(int precision) {
        setPrecision(new MathContext(precision, RoundingMode.HALF_UP));
    }
...
//        MathContext locmc = new MathContext(mc.getPrecision()+2,mc.getRoundingMode()) ;
        ...
        while (!(Math.abs(BigDecimal.ONE.subtract(x.divide(s.pow(2, context), context)).doubleValue()) < eps)) {
             ...
             */
            s = s.add(x.divide(s, context)).divide(half, context);
            /* debugging
...
//        MathContext mc = new MathContext( 2+x.precision() ) ;
        ...
        for (;;)
        {
             ...
             */
            BigDecimal c = xhighpr.divide( s.pow(n-1),context ) ;
            c = s.subtract(c) ;
//            MathContext locmc = new MathContext( c.precision() ) ;
            c = c.divide(nth,context) ;
            s = s. subtract(c) ;
...
//        mc = new MathContext(  err2prec(z.doubleValue(),0.5*zerr/z.doubleValue() ) ) ;
     ...
     */
    static final private int TAYLOR_NTERM = 8 ;
...
//            MathContext mc = new MathContext( invx.precision() ) ;
            return BigDecimal.ONE.divide( invx, context ) ;
        }
...
//                MathContext mcTay = new MathContext( err2prec(1.,xUlpDbl/TAYLOR_NTERM) ) ;
                for(int i=1 ; i <= TAYLOR_NTERM ; i++)
                {
                    ...
                    final BigDecimal c= xpowi.divide(new BigDecimal(ifac),context)  ;
                    resul = resul.add(c) ;
...
//                    MathContext mctmp = new MathContext( expxby10.precision()-exsub+2 ) ;
                    ...
                    while ( exsub-- > 0 )
                        pex *= 10 ;
                    expxby10 = expxby10.pow(pex,context) ;
                }
...
//            MathContext mc = new MathContext( x.precision() - (int)Math.log10((double)(Math.abs(n))) ) ;
            if ( n > 0 )
                return x.pow(n,context) ;
            else
                return BigDecimal.ONE.divide( x.pow(-n),context) ;
        }
    ...
    static public BigDecimal sin(final BigDecimal x)
    {
        ...
        else
        {
            ...
            if ( res.compareTo(pi()) > 0 )
            {
                /* pi<x<=2pi: sin(x)= - sin(x-pi)
                 */
                return sin(subtractRound(res, pi())) .negate() ;
            }
            else if ( res.multiply(new BigDecimal("2")).compareTo(pi()) > 0 )
            {
                /* pi/2<x<=pi: sin(x)= sin(pi-x)
                 */
                return sin(subtractRound(pi(),res)) ;
            }
            else
            {
                 ...
                 */
                if ( res.multiply(new BigDecimal("4")).compareTo(pi()) > 0 )
                {
                    /* x>pi/4: sin(x) = cos(pi/2-x)
                     */
                    return cos( subtractRound(pi().divide(new BigDecimal("2"), context),res) ) ;
                }
                else
                {
...
//                    MathContext mcTay = new MathContext( err2prec(res.doubleValue(),xUlpDbl/k) ) ;
                    for(int i=1 ; ; i++)
                    {
                        ...
                        BigDecimal corr = xpowi.divide(new BigDecimal(ifac),context) ;
...
//            mc = new MathContext( x.precision() ) ;
            ...
            else
            {
                ...
                if ( res.multiply(new BigDecimal("4")).compareTo(p) > 0 )
                {
                     ...
                     */
                    return sin( subtractRound(p.divide(new BigDecimal("2"), context),res) ) ;
                }
                else
                {
...
//            MathContext mc = new MathContext( err2prec(3.14159,errpi) ) ;
            return pi().divide(new BigDecimal(2), context) ;
        }
        else if ( x.compareTo(BigDecimal.ZERO) < 0 )
        {
...
//            MathContext mc = new MathContext( resul.precision() ) ;
            BigDecimal pihalf = pi().divide(new BigDecimal(2), context) ;

//            mc = new MathContext( err2prec(resul.doubleValue(),eps) ) ;
            return pihalf.subtract(resul,context) ;
        }
        else
        {
...
//        mc = new MathContext( err2prec(resul.doubleValue(),eps) ) ;
    ...
    static public BigDecimal atan(final BigDecimal x)
    {
        ...
        else if ( x.doubleValue() >0.7 && x.doubleValue() <3.0)
        {
            ...
            return multiplyRound( atan(newx), 2) ;
...
//            MathContext mc = new MathContext( 2+err2prec(3.1416,eps) ) ;
            BigDecimal onepi= pi() ;
            BigDecimal resul = onepi.divide(new BigDecimal(2), context) ;
    ...
    static public BigDecimal sinh(final BigDecimal x)
    {
        ...
        else
        {
            if ( x.doubleValue() > 2.4 )
            {
                ...
                BigDecimal xhalf = x.divide(two, context) ;
                return sinh(xhalf).multiply(cosh(xhalf)).multiply(two) ;
                /* The error in the result is set by the error in x itself.
...
//            MathContext mc = new MathContext( err2prec(Math.tanh(x.doubleValue()),eps) ) ;
            return BigDecimal.ONE.subtract(exp2x).divide( BigDecimal.ONE.add(exp2x), context) ;
        }
    } /* BigDecimalMath.tanh */
    ...
    static public BigDecimal asinh(final BigDecimal x)
    {
        ...
        else
        {
             ...
             */
            return log(hypot(1,xhighpr).add(xhighpr)) ;
...
//            return logx;//.round(mc) ;
    ...
    static public BigDecimal acosh(final BigDecimal x)
    {
        ...
        else
        {
             ...
             */
            return log( sqrt(xhighpr.pow(2).subtract(BigDecimal.ONE) ) .add(xhighpr)) ;
...
//        MathContext mc = new MathContext( 2+err2prec(3.1416,errpi) ) ;
        BigDecimal onepi= pi() ;
        BigDecimal pihalf = onepi.divide(new BigDecimal(2), context) ;
...
//        return resul;//.round(mc) ;
    ...
    {
        return x.multiply(new BigDecimal(n)) ;
...
//        return resul;//.round(mc) ;
    ...
    static public BigDecimal divideRound(final BigDecimal x, final BigDecimal y)
    {
         ...
         */
//        MathContext mc = new MathContext( Math.min(x.precision(),y.precision()) ) ;
        BigDecimal resul = x.divide(y,context) ;
         ...
         */
        return scalePrec(resul,context) ;
    }
    ...
    static public BigDecimal divideRound(final BigDecimal x, final int n)
    {
         ...
         */
//        MathContext mc = new MathContext( x.precision() ) ;
        return x.divide(new BigDecimal(n),context) ;
    }
...
//        MathContext mc = new MathContext( x.precision() ) ;
    ...
    static public BigDecimal divideRound(final BigInteger n, final BigDecimal x)
    {
         ...
         */
//        MathContext mc = new MathContext( x.precision() ) ;
        return new BigDecimal(n).divide(x,context) ;
    } /* divideRound */
...
//        MathContext mc = new MathContext( x.precision() ) ;
    ...
    {
        return x.setScale(d+x.scale(), context.getRoundingMode()) ;
    }
